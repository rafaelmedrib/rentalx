...
let RefreshTokenUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(token) {
    ...
    const userToken = await this.usersTokensRepository.findByUserIdAndByRefreshToken(sub, token);
    ...
    const expiration_date = this.dateProvider.addDays(_auth.default.expires_in_refresh_token_days);
    ...
    const newToken = (0, _jsonwebtoken.sign)({}, _auth.default.secret_token, {
      subject: sub,
      expiresIn: _auth.default.expires_in
    });
    return {
      refresh_token,
      token: newToken
